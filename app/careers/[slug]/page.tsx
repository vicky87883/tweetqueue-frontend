...
export default async function JobPage({ params }: JobPageProps) {
  ...
  const numericSalary = job.salary.match(/\$([\d,]+)/g)?.map((value) =>
      ...
      ? {
          baseSalary: {
            ...
            value: {
              ...
              minValue: numericSalary[0],
              maxValue: numericSalary[1],
              unitText: job.salary.includes('hour') ? 'HOUR' : 'YEAR',
            },
          },
        }
      : {}),
      ...
      <section className="px-4 py-8 sm:px-6 sm:py-12">
        <div className="mx-auto grid max-w-4xl gap-6 lg:grid-cols-[1fr_18rem]">
          ...
          <aside className="h-fit rounded-3xl border border-gray-800 bg-zinc-950 p-6">
            ...
            <a
              href={`mailto:careers@tweetqueue.com?subject=${encodeURIComponent(job.title)}`}
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-white px-5 py-3 font-semibold text-black hover:bg-gray-200"
            >
